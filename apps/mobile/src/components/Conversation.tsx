...
import { ArrowUp, ChevronDown, Paperclip, Settings2, Square, X } from "lucide-react";
...
export function Conversation({ controller, view }: { controller: MobileController; view: MobileView }) {
  ...
  return <main className="conversation">
    <div className="conversation-controls"><button className="config-chip" onClick={() => setConfigPanel("mode")} disabled={!session || view.configuring}><Settings2 size={15}/>{modeLabel}<ChevronDown size={14}/></button><button className="config-chip model-chip" onClick={() => setConfigPanel("model")} disabled={!session || view.configuring}><span>{modelLabel}</span><ChevronDown size={14}/></button></div>
    ...
    {!stick.current && view.messages.length > 0 && <button className="back-latest" onClick={() => { stick.current = true; scroll.current?.scrollTo({ top: scroll.current.scrollHeight, behavior: "smooth" }); }}>{t("backToLatest")}</button>}
    <form className="composer" onSubmit={submit}>
      ...
      <textarea ref={composerInput} aria-label={t(imageMode ? "imagePrompt" : "prompt")} placeholder={t(session?.capabilities.canPrompt === false ? "readOnly" : imageMode ? "imagePrompt" : "prompt")} value={draft} rows={1} onChange={(event) => updateDraft(event.target.value)} disabled={view.busy || Boolean(view.uncertainMessageId) || session?.capabilities.canPrompt === false}/>
    ...
    <SessionConfigSheet open={Boolean(configPanel)} panel={configPanel ?? "mode"} close={() => setConfigPanel(undefined)} controller={controller} view={view}/>
  </main>;
}
