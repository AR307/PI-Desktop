...
/** Keeps the MC mobile device secret in Electron main, encrypted at rest. */
export class MobileDeviceCredentials {
  private writes: Promise<void> = Promise.resolve();

  constructor(
    private readonly path: string,
    private readonly encryption: MobileDeviceEncryption,
  ) {}
  ...
  save(value: MobileDeviceCredentialsRecord): Promise<void> {
    ...
    const write = this.writes
      ...
      .catch(() => {
        throw new Error("device_credential_write_failed");
      });
    this.writes = write.catch(() => undefined);
    return write;
  }

  /** Write an encrypted tombstone so a clear never leaves plaintext secrets. */
  clear(): Promise<void> {
    this.assertAvailable();
