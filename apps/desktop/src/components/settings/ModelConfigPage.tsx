...
export function ModelConfigPage() {
  ...
  // MirrorCoding rows live in their own account section below; the shared
  // service list only manages API services and vendor subscriptions.
  const serviceProviders = useMemo(
    () => providers.filter((provider) => provider.authKind !== "mirrorcoding"),
    [providers],
  );
        ...
        <div className="model-config-section-head">
          <div className="settings-card-heading-line">
            ...
            {serviceProviders.length > 0 ? (
              <span className="provider-section-count">{serviceProviders.length}</span>
            ) : null}
          </div>
