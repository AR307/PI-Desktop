...
export function MirrorCodingDefault() {
  ...
  const models = [...new Set(providers.flatMap((provider) => provider.models.map((model) => model.id)))];
  const [modelId, setModelId] = useState("");
  const [providerId, setProviderId] = useState("");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState(false);
  if (!settings || settings.defaultProviderId || !models.length) return null;
  const groups = providers.filter((provider) => provider.models.some((model) => model.id === modelId));
  const save = async () => {
